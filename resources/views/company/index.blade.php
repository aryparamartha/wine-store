...
@section('title', 'WineryApp - Companies')
...
<script>
    $(document).ready(function () {
        ...
        $('#btn-add-company').click(function () {
            ...
            $('#company-form').attr('action', "{{route('company.store')}}");
        ...
        $('.btn-edit-company').click(function() {
            ...
            $('#company-form').attr('action', '/company/update/' + company.id);
            $('#company-modal-title').html("Update Company");
            $('#company-modal').modal('show');
            $('#name').val(company.name);
            $('#address').val(company.address);
            $('#number').val(company.number);
            $('#email').val(company.email);
            $('#pic').val(company.pic).trigger('change');
        });
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row mb-20">
                        ...
                        <div class="col-md-6 col-6">
                            <div class="flt-right">
                                <a class="btn btn-success btn-icon-text btn-edit-profile" href="javascript:void(0)" id="btn-add-company" >
                                    <i data-feather="plus" class="btn-icon-prepend"></i> Add Company
                                </a>
                            </div>  
                        </div>
                    </div>          
                        ...
                        <table id="dataTableExample" class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button data-index="{{$key}}" class="btn-edit-company btn btn-primary btn-icon">
                                            <i data-feather="edit"></i>
                                        </button>
                                        <form class="frm-dlt-alert" action="{{route('company.delete', $company)}}" method="post" style="display: inline-block;">
                                            @csrf
                                            <button type="button" class="btn-dlt-alert btn btn-danger btn-icon" data-title="Delete Company" data-text="Are you sure you want to delete this data?">
...
<div class="modal fade" id="company-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        ...
        <form id="company-form" class="forms-sample" action="" method="POST">
            @csrf
            <div class="modal-header">
                <h5 class="modal-title" id="company-modal-title"></h5>
